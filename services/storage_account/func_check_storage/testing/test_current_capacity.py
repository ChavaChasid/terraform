...
class mock_time_series:
    def __init__(self, last_total):
        self.data = [
            mock_object_with_total(5),
            mock_object_with_total(5),
            mock_object_with_total(123456789),
            mock_object_with_total(last_total),
        ]
...
class mock_metrics:
    def list(self, storage_account_id, metricnames, timespan, interval, aggregation):
        return (
            ...
            and metricnames == "UsedCapacity"
            and timespan == "2023-11-28T00:00:00Z/2023-11-29T00:00:00Z"
            and interval == "PT1H"
            and aggregation == "Total"
        )


@patch(
    "project.current_capacity.create_storage_management_client",
    Mock(return_value=mock_storage_client()),
)
@patch(
    "project.current_capacity.create_monitor_management_client",
    Mock(return_value="monitor_client"),
)
@patch(
    "project.current_capacity.get_metrics_list",
    Mock(return_value=mock_mertrics_data("value")),
)
@patch(
    "project.current_capacity.return_first_row",
    Mock(side_effect=[mock_object_with_timeseries(), mock_time_series(456789123)]),
)
...
@patch(
    "project.current_capacity.return_first_row",
    Mock(side_effect=[mock_object_with_timeseries(), mock_time_series(None)]),
)
...
def test_find_total_capacity():
    assert (
        find_total_capacity(
            [
                mock_object_with_total(None),
                mock_object_with_total(5),
                mock_object_with_total(None),
                mock_object_with_total(3),
                mock_object_with_total(None),
            ]
        )
        == 3
...
@patch(
    "project.current_capacity.datetime.date",
    Mock(**{"today.return_value": date(2023, 11, 29)}),
)
@patch(
    "project.current_capacity.datetime.timedelta", Mock(return_value=timedelta(days=1))
)
...
def test_return_first_row():
    assert return_first_row([{"name": "hello"}, {"second_name": "world"}]) == {
        "name": "hello"
    }
