...
from project.last_fetch_time import (
    check_last_fetch_is_early,
    get_last_fetch_time_from_documentation,
    check_creation_date_is_early,
    get_creation_date,
    should_alert,
)
...
@patch("project.last_fetch_time.time_index_for_check_last_fetch", "Day")
@patch("project.last_fetch_time.time_period_for_check_last_fetch", "3")
def test_check_last_fetch_is_early_return_last_fetch_time():
    assert check_last_fetch_is_early(
        "storage_client", "resource_group_name", "storage_account_name", "2023-11-12"
    ) == {"last_fetch_time": "2023-11-12", "alert": False}


@patch("project.last_fetch_time.time_index_for_check_last_fetch", "Day")
@patch("project.last_fetch_time.time_period_for_check_last_fetch", "3")
@patch(
    "project.last_fetch_time.get_last_fetch_time_from_documentation",
    Mock(return_value="12-04-2024"),
)
def test_check_last_fetch_is_early_return_false_true():
    assert check_last_fetch_is_early(
        "storage_client", "resource_group_name", "storage_account_name", False
    ) == {"last_fetch_time": "12-04-2024", "alert": False}
...
def test_check_last_fetch_is_early_return_false_false():
    assert check_last_fetch_is_early(
        "storage_client", "resource_group_name", "storage_account_name", False
    ) == {"last_fetch_time": False, "alert": False}


@patch("project.last_fetch_time.get_date", Mock(return_value=date(2023, 10, 29)))
@patch(
    "project.last_fetch_time.get_documentation_entities_by",
    Mock(return_value="query_entities"),
)
@patch(
    "project.last_fetch_time.get_nearest_last_fetch_time_entity",
    Mock(return_value="required_entity"),
)
def test_get_last_fetch_time_from_documentation():
    assert (
        get_last_fetch_time_from_documentation("storage_account_name", "freq_test")
        == "required_entity"
    )
...
def test_get_last_fetch_time_from_documentation_failed_exception():
    assert not get_last_fetch_time_from_documentation(
        "storage_account_name", "freq_test"
    )


@patch(
    "project.last_fetch_time.get_creation_date", Mock(return_value=date(2023, 11, 28))
)
...
def test_check_creation_date_is_early_called_get_creation_date(get_creation_date):
    ...
    get_creation_date.assert_called_once_with(
...
@patch(
    "project.last_fetch_time.convert_to_date_type_mil_seconds", Mock(return_value=False)
)
...
@patch("project.last_fetch_time.get_date", Mock(return_value=date(2023, 10, 29)))
@patch(
    "project.last_fetch_time.convert_datetime_to_date",
    Mock(side_effect=[date(2023, 10, 29), date(2023, 11, 28)]),
)
@patch(
    "project.last_fetch_time.calculate_diff_between_two_dates", Mock(return_value=30)
)
...
@patch(
    "project.last_fetch_time.convert_datetime_to_date",
    Mock(side_effect=[date(2023, 10, 15), date(2023, 10, 20)]),
)
@patch(
    "project.last_fetch_time.calculate_diff_between_two_dates", Mock(return_value=-5)
)
