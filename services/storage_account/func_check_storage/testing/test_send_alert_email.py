...
from project.send_alert_email import (
    ...
    create_object_for_alerts_documentation_table,
)


@patch(
    "project.send_alert_email.add_entity_to_alerts_documentation",
    Mock(return_value=None),
)
@patch("project.send_alert_email.requests", Mock(return_value=None))
@patch(
    "project.send_alert_email.connection_string_managers", "connection_string_managers"
)
@patch("project.send_alert_email.managers_table", "managers_table")
...
@patch(
    "project.send_alert_email.retrieve_data_from_table", Mock(side_effect=Exception())
)
...
def test_main_alerts_assert_manager_information_raise_exception(
    add_entity_to_alerts_documentation,
):
    main_alerts("storage_name", "email_body", 1, 1, "subscription_name")
    add_entity_to_alerts_documentation.assert_called_once_with(
        {"subName": "null", "subManagerMail": "null"},
        "storage_name",
        "email_body",
        1,
        1,
...
@patch(
    "project.send_alert_email.create_object_for_alerts_documentation_table",
    Mock(
        return_value={
            "PartitionKey": "3",
            "RowKey": "3",
...
def test_create_object_for_alerts_documentation_table():
    assert create_object_for_alerts_documentation_table(
        1,
        1,
        "sender_date",
        "sub_name",
